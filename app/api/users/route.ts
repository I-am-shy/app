...
export async function GET(request: NextRequest) {
  ...
  if(data){
    ...
    return NextResponse.json({ msg: "用户不存在",status: 400 }, { status: 400 })
  }
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ msg: error.message,status: 400 }, { status: 400 })
  }
}
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    if (data === true) {
      return NextResponse.json({ msg: '更新用户成功',status: 200 }, { status: 200 })
    } else {
      return NextResponse.json({ msg: data,status: 500 }, { status: 500 })
    }
  } catch (error:any) {
